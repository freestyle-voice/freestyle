...
`wav`, `mp3`, `m4a`, `ogg`, and `flac`. Files are decoded and resampled to 16 kHz mono in the browser before being sent to the server, so any common format works regardless of the provider.
