...
On first run the plugin seeds a few ready-to-use commands so it works out of the box. They all use the cross-platform **Open URL / app** action — no keys, scripts, or setup required:

| Command | Say | Opens |
| --- | --- | --- |
| **Web search** | _"search the web for…"_, _"search the internet for…"_ | a Google search for the payload |
| **YouTube search** | _"search youtube for…"_, _"find on youtube…"_ | a YouTube search |
| **Maps directions** | _"get directions to…"_, _"navigate to…"_ | Google Maps for the destination |
| **Wikipedia lookup** | _"look up on wikipedia…"_, _"search wikipedia for…"_ | a Wikipedia search |

Edit, disable, or delete them freely — they're seeded only once, when no commands have ever been saved, so anything you remove won't reappear.
