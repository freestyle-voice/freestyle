...
export function UpdateBanner({
  ...
  useEffect(() => {
    const applyStatus = (status: {
      ...
      setDownloading(status.downloadState === "downloading");
      setUpdateDownloaded(status.downloadState === "downloaded");
      if (status.downloadState !== "downloading") setUpdateError(null);
    };
    const removeStatus = window.api?.onUpdateStatus?.(applyStatus);
    void window.api
      ?.getUpdateStatus?.()
      .then(applyStatus)
      .catch(() => {});
