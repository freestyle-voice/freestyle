...
/**
 * The billing page is an account surface, not an upsell, once a member is
 * Pro. Keep plan comparisons in the upgrade flow and give paid members one
 * clear place to manage their subscription instead.
 */
export function ProMembership({
  ...
  PricingPlansProps,
  "openBillingPortal" | "portalOpening"
>): React.JSX.Element {
  const [portalError, setPortalError] = useState<string | null>(null);

  const manageSubscription = useCallback(() => {
    setPortalError(null);
    void openBillingPortal().then((opened) => {
      if (!opened) {
        setPortalError(
  ...
  return (
    <section
      className="glass-card overflow-hidden rounded-[12px] border"
      ...
      <div className="px-5 py-4">
        <div>
          <div className="flex items-baseline gap-2.5">
            ...
            <Badge className="mono h-4 px-1.5 text-[9px] uppercase tracking-[0.1em]">
              Active
            </Badge>
          </div>
          <p className="text-muted-foreground mt-1 text-[12.5px] leading-[1.5]">
            Everything in Pro is ready across your devices.
          </p>
        </div>
      </div>

      <div className="border-border grid border-t min-[760px]:grid-cols-[minmax(0,1fr)_minmax(220px,0.72fr)]">
        <div className="px-5 py-4">
          <p className="mono text-muted-foreground text-[9.5px] font-medium uppercase tracking-[0.13em]">
            Included
          </p>
          <ul className="mt-3 grid gap-x-5 gap-y-2 min-[980px]:grid-cols-2">
            {PRO_MEMBERSHIP_FEATURES.map((feature) => (
              <li
                key={feature}
                className="text-muted-foreground flex items-start gap-1.5 text-[12px] leading-snug"
              >
        ...
        <div className="border-border flex flex-col justify-center border-t px-5 py-4 min-[760px]:border-t-0 min-[760px]:border-l">
          <p className="text-foreground text-[13px] font-medium">
            Billing and invoices
          </p>
          <p className="text-muted-foreground mt-1 text-[11.5px] leading-[1.5]">
            Payment details, invoices, and subscription changes.
          </p>
          <Button
            size="sm"
          ...
          {portalError ? (
            <p className="text-destructive mt-2 text-[11px] leading-snug">
              {portalError}
            </p>
          ) : null}
        </div>
