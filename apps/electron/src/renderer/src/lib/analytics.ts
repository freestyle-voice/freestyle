...
/**
 * Capture a structured Sentry log from the renderer.
 *
 * Events are POSTed to the server's `/api/telemetry`, which forwards to the
 * same server-side `capture()` (apps/server/src/lib/sentry.ts) that every
 ...
 * and applies the same release and account context — the renderer ships no SDK.
 *
