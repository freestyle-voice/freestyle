...
export default function AppPage(): React.JSX.Element {
  ...
  const setPillState = useCallback((next: PillState) => {
    ...
    setState(next);
    if (next === "idle" || next === "recording" || next === "transcribing") {
      window.api?.setDictationPhase?.(next);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    getClient()
      ...
      .then((settings) => {
        ...
        setCancelMode(normalizePillCancelMode(settings.pill_cancel_button));
    ...
    const removePillPos =
      window.api?.onPillPositionChanged?.(applyPillPosition);
    ...
    const removeAudioDucking = window.api?.onAudioDuckingChanged?.(
      (enabled) => {
