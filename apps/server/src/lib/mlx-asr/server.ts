...
async function startWorker(modelId: string): Promise<void> {
  ...
  const args = ["--model", def.hfId];
  let command = workerPath;
  let spawnArgs = args;
  ...
  if (!existsSync(workerPath)) {
    ...
    if (!isMlxAudioInstalled(python)) {
      throw new Error(
        `MLX ASR Python dependencies are not installed for ${python}. Run: ${python} -m pip install mlx-audio`,
      );
    }

    const scriptPath = getMlxAsrServerScriptPath();
    if (!existsSync(scriptPath)) {
      throw new Error(`MLX ASR worker script not found: ${scriptPath}`);
    }
    command = python;
  ...
  proc.stderr?.on("data", (data: Buffer) => {
    ...
    if (text && isDev) console.log("[mlx-asr]", text);
  });
  ...
  proc.on("close", (code) => {
    if (workerProcess !== proc) return;
    const err = new Error(
      `mlx-asr worker exited unexpectedly: exit code ${code}`,
    );
    failWorker(err);
  });
